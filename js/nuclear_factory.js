...
function preload() {
	...
	game.loadBackgroundImage("background_1", "img/nuclear_factory/Lab background.png");
	...
	//obstacle textures
	stoneTexture = new Sprite("img/nuclear_factory/Table 2.0.png");
	treeTexture = new Sprite("img/nuclear_factory/Toxic Waste 2.0.png");
	obstacleTextures.push(stoneTexture);
	obstacleTextures.push(treeTexture);
	
	cokeWidth = 128;
	cokeHeight = 128;
	cokeTexture = new Sprite("img/nuclear_factory/coin-rotation.png", cokeWidth, cokeHeight);
...
function newRunnerItem(laneIndex, texture, xOffset, y, width, height, velocity, allItems) {
	...
	if (texture == cokeTexture) {
		runnerItem.addAnimation('rotate', [0, 1, 2, 3, 2, 1], 10);
		runnerItem.playAnimation('rotate');
	}
...
function newObstacle(laneIndex) {
	newRunnerItem(laneIndex, randomObstacleTexture(), -25, 0, 100, 100, OBSTACLE_VEL_Y + (duration * 0.5), obstacles);
}

function newPickUp(laneIndex) {
	newRunnerItem(laneIndex, randomPickUpTexture(), 0, 0, 25, 25, PICKUP_VEL_Y, pickUps);
}
...
function create() {
	...
	var colaScoreImgX = game.gameWidth() - 65;
	var colaScoreImgY = 10;
	colaScoreImg = cokeTexture.create(colaScoreImgX, colaScoreImgY, cokeWidth / 2, cokeHeight / 2);
	colaScoreImg.addAnimation('rotate', [0, 1, 2, 3, 2, 1], 10);
	colaScoreImg.playAnimation('rotate');
	colaScoreText = new Text("0", colaScoreImgX - 40, colaScoreImgY + 10, "37px", "Gaegu", "#ffffff");
...
function updateObstacles() {
	for (i = 0; i < obstacles.length; i++) {
		...
		game.checkCollision(o, idlePlayer, function() {
			o.setVelocityY(OBSTACLE_VEL_Y + (duration * 0.5));
			if (pushBack == null) {
...
function updateScore() {
	colaScoreText.changeText(pickUpScore.cola);
}
...
function handleLeaderboards() {
	...
	levelScores.push(String(pickUpScore.cola));
